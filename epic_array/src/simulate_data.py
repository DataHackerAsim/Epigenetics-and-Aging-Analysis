...
_CLOCK_COEFFICIENT_FILES = [
    ...
    # Note: AltumAge (deep NN) and StocP/StocH share CpGs with the linear
    # clocks above, so the direction vote here also drives their signal.
]
...
def simulate_methylation_dataset(
    ...
    slopes = np.zeros(len(cpg_index), dtype=np.float64)
    for cpg, sign in directions.items():
        if cpg in cpg_idx:
            slopes[cpg_idx[cpg]] = slope_magnitude * sign

    # Build matrix column-by-column. Use float64 + writeable copy so
    # in-place normalisation steps in clocks (e.g. DunedinPACE quantile
    # normalisation) don't trip over read-only buffers.
    M = np.empty((len(cpg_index), n_samples), dtype=np.float64)
    for j, age in enumerate(ages):
        age_centered = float(age) - 50.0
        noise = rng.normal(0.0, noise_sd, size=len(cpg_index))
        M[:, j] = np.clip(baseline + slopes * age_centered + noise, 0.005, 0.995)

    methylation_df = pd.DataFrame(np.array(M, copy=True), index=cpg_index, columns=sample_ids)
    metadata_df = pd.DataFrame(
